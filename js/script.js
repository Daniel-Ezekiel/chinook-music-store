...
const showDeleteModal = (e) => {
  e.preventDefault();
  //   console.log(e);
  // add a class of active to the delete confirmation modal
  modalEl.classList.add("active");

  //   grabbing the album details from the dataset object
  const albumId = e.target.dataset.albumId;
  const albumTitle = e.target.dataset.albumTitle;
  const artistName = e.target.dataset.artistName;

  //   console.log(albumId, albumTitle, artistName);
...
// setup and event listener to handle clicking for each button
btnsDelete.forEach((btn) => btn.addEventListener("click", showDeleteModal));

// setup if user confirm to delete album using the modal
formModalDelete.addEventListener("submit", (e) => {
  ...
  const isConfirmedDelete =
    ...
    artistName &&
    confirm(
      `Are you sure you want to delete Album ${albumId}: ${albumTitle} by ${artistName}?`
    );
